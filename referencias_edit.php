...
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_verifica = sprintf("SELECT * FROM referencias WHERE rfc_referencia = %s", GetSQLValueString($colname_verifica, "text"));
...
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_verifica2 = sprintf("SELECT * FROM referencias WHERE nombre_referencia = %s", GetSQLValueString($colname_verifica, "text"));
...
//if ($_POST['activo_proveedor']<>1) {$_POST['activo_proveedor']=0;}
  ...
  $insertSQL = sprintf("INSERT INTO referencias (nombre_referencia, clave_cliente, rfc_referencia, persona, domicilio_referencia, ciudad_referencia, cp_referencia, tel_referencia, tel_referencia_movil, tel_referencia_trabajo, fax_referencia, email_referencia) VALUES (%s , %s, %s, %s , %s, %s, %s, %s, %s, %s, %s, %s)",
...
//                       GetSQLValueString($_POST['clave_cliente'], "text"),					   
                       ...
                       GetSQLValueString($_POST['tel_referencia_movil'], "text"),
                       GetSQLValueString($_POST['tel_referencia_trabajo'], "text"),
...
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_verifica = sprintf("SELECT * FROM referencias WHERE rfc_referencia = %s AND clave_referencia<> %s", GetSQLValueString($colname_verifica, "text"),GetSQLValueString($colname_verifica2, "text") );
$verifica = mysqli_query($contratos_londres, $query_verifica) or die(mysqli_error($contratos_londres));
$row_verifica = mysqli_fetch_assoc($verifica);
$totalRows_verifica = mysqli_num_rows($verifica);
mysqli_free_result($verifica);
...
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_verifica2 = sprintf("SELECT * FROM referencias WHERE nombre_referencia = %s AND clave_referencia<> %s", GetSQLValueString($colname_verifica, "text"),GetSQLValueString($colname_verifica2, "text") );
$verifica2 = mysqli_query($contratos_londres, $query_verifica2) or die(mysqli_error($contratos_londres));
$row_verifica2 = mysqli_fetch_assoc($verifica2);
$totalRows_verifica2 = mysqli_num_rows($verifica2);
mysqli_free_result($verifica2);
...
// if ($_POST['activo_proveedor']<>1) {$_POST['activo_proveedor']=0;}
 ...
 $updateSQL = sprintf("UPDATE referencias SET nombre_referencia=%s, rfc_referencia=%s, persona=%s, domicilio_referencia=%s, ciudad_referencia=%s, cp_referencia=%s, tel_referencia=%s, tel_referencia_movil=%s, tel_referencia_trabajo=%s, fax_referencia=%s, email_referencia=%s WHERE clave_referencia='$recordID2' AND clave_cliente='$recordID'",
                       GetSQLValueString($_POST['nombre_referencia'], "text"),
					   GetSQLValueString($_POST['rfc_referencia'], "text"),
                       GetSQLValueString($_POST['persona'], "text"),
					   GetSQLValueString($_POST['domicilio_referencia'], "text"),
...
//	header(sprintf("Location: %s", $updateGoTo));
 ...
 $updateSQL = sprintf("DELETE from referencias WHERE clave_referencia='$recordID2'");
...
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_referencia2 = "SELECT * FROM referencias WHERE clave_referencia='$recordID2' AND clave_cliente='$recordID'";
...
<p class="style2"><strong>Referencias.</strong></p>
...
do {  
?>
        <option value="<?php echo $row_cliente_combo['clave_cliente']?>"<?php if (!(strcmp($row_cliente_combo['clave_cliente'], ($recordID2>0) ? $row_referencia2['clave_cliente'] :$recordID))) {echo "selected=\"selected\"";} ?>><?php echo $row_cliente_combo['nombre_cliente']?></option>
        <?php
...
?>    
        ...
        <?php //echo "Cliente: ".$recordID." Referencia: ".$recordID2; ?></td>
      </tr>
    <tr valign="baseline">
    ...
    <tr valign="baseline">
      ...
      <td class="Texto_tabla"><input name="tel_referencia_movil" type="text" class="style3 " value="<?php if ($parametro1 = 0) { echo '" "'; } else  { echo $row_referencia2['tel_referencia_movil']; }?>" size="65" /></td>
      </tr>
      ...
      <td class="Texto_tabla"><input name="tel_referencia_trabajo" type="text" class="style3 " value="<?php if ($parametro1 = 0) { echo '" "'; } else  { echo $row_referencia2['tel_referencia_trabajo']; }?>" size="65" /></td>
      </tr>
