...
$recordID5=$_POST['clave_empresa'];

//echo "EMPRESA.......................:".$recordID5."<BR>";


//$_GET['clave_empresa'] = $recordID0;
...
$filtro=" where clave_contrato='$recordID'";
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_pagares = "select clave_pagare, numero_pagare, DATE_FORMAT(fecha_pagare, '%d-%m-%Y') AS fecha_pagare, DATE_FORMAT(vence_pagare, '%d-%m-%Y') AS vence_pagare, importe_pagare, clave_contrato, pagado, vencido, clave_empresa from pagares".$filtro;
...
$clave_temporal=$row_pagares ['clave_empresa'];

if ($clave_temporal<=0)
	$clave_temporal=$recordID1;

//echo "CLAVE DE LA EMPRESA: ".$clave_temporal;
...
$tpagares = mysqli_query($contratos_londres, $query_tpagares) or die(mysql_error());
...
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_emp2 = "select * from empresa";
$emp2 = mysqli_query($contratos_londres, $query_emp2) or die(mysql_error());
$row_emp2 = mysqli_fetch_assoc($emp2);
$totalRows_emp2 = mysqli_num_rows($emp2);
...
$suma_pagares=0;
//suma de la cantidad a pagar de los pagares...
$suma_pagares=$row_tpagares["suma_pagares"];
//suma de el numero de pagares...
$nos_pagares=$row_tpagares["nos_pagares"];
//Total a pagar del contrato...
$total_del_contrato=$row_contra["ctotal"];
...
if (($numero_pagares<>$nos_pagares)&&($nos_pagares<>0)){
  ...
  echo "<SCRIPT language=\"JavaScript\">
	...
	//Si la cantidad total del contrato difiere de la cantidad total de los pagares, entonces, preguntar si se hace de nuevo...
	
	if ((number_format($suma_pagares,2)<>number_format($total_del_contrato,2))&&($nos_pagares<>0)){
		//echo "Suma pagares: ".number_format($suma_pagares,2)."<BR>";
		//echo "Total del contrato: ".number_format($total_del_contrato,2)."<BR>";
		//echo "numero de pagares: ".$nos_pagares."<BR>";
		$tempo1=$recordID;
		/*
		$mensaje="suma_pagares= ".number_format($suma_pagares,2)." total_del_contrato= ".number_format($total_del_contrato,2)." nos_pagares= ".$nos_pagares;
	...
	*/
		...
		echo "<script language='javascript'> var $tempo1 = <?php echo '$tempo1'; ?>; </script> ";
		echo "<script language='javascript'>
		if(!confirm('El total a pagar de los Pagares difiere del total a pagar en el contrato, Quieres Crearlos nuevamente? ')) 
		{ 
			//location.href='contrato.php?parametro1='+$tempo1; 
		} 
		</script> ";
...
for ($i=1;$i<=$numero_pagares;$i++) {
	$suma_pagares=$suma_pagares+$_POST['i'.$i];
}
for ($i=1;$i<=$numero_pagares;$i++) {
	...
	//$dia=$dia_primer_pago;
	// analizar si es bisiesto y corregir fecha si es asi...
	$dia=substr($_POST['v'.$i],0,2);
...
/////////////////////
	...
	//echo "clave_contrato=".$recordID."<BR>";
	//echo "importe del pagare: ".$i." = ".$_POST['i'.$i]."<BR>";
	//$importe_temporal=$_POST['i'.$i];
	//$empresa_temporal=$_POST['clave_empresa'];
	
	
	$updateSQL = sprintf("UPDATE pagares SET vence_pagare=%s, importe_pagare=%s, clave_empresa=%s WHERE clave_contrato='$recordID' AND numero_pagare='$i'",
                       GetSQLValueString($fechaT, "date"),
					   GetSQLValueString($_POST['i'.$i], "text"),
					   GetSQLValueString($_POST['clave_empresa'], "text"));	
 	mysqli_select_db($contratos_londres, $database_contratos_londres);
 	$Result1 = mysqli_query($contratos_londres, $updateSQL) or die(mysql_error());
	//echo $suma_pagares."<BR> ,resultado: ".$Result1."  sql: ".$updateSQL."<BR>";
	//printf("Registros actualizados: %d\n", mysql_affected_rows());
	
mysqli_select_db($contratos_londres, $database_contratos_londres);
$query_numeros_tempo = "select * from pagares WHERE clave_contrato='$recordID' AND numero_pagare='$i'";
$numeros = mysqli_query($contratos_londres, $query_numeros_tempo) or die(mysql_error());
$row_numeros = mysqli_fetch_assoc($numeros);
$totalRows_numeros = mysqli_num_rows($numeros);

mysqli_free_result($numeros);
	
	
	if (mysqli_affected_rows($contratos_londres)==0 && $totalRows_numeros==0){
...
/*				$mensaje='FECHA PRIMER PAGO: '.$fecha_del_primerpago ;
			  echo "<script language='javascript'> alert('$mensaje'); </script> "; */
			}
			else
			{
...
/*				$mensaje='FECHA DEL PAGO: '.$fechaVence." FECHA DEL PRIMER PAGO: ".$fecha_del_primerpago ;
			  echo "<script language='javascript'> alert('$mensaje'); </script> "; */
			}
			
		$insertSQL = sprintf("INSERT INTO pagares (numero_pagare, fecha_pagare, vence_pagare, importe_pagare, clave_contrato, pagado, vencido, clave_empresa) VALUES (%s , %s, %s, %s, %s, %s, %s, %s)",
...
#forma_pagare table tr .style6 p .style4 {
	text-align: right;
}
#forma_pagare table tr .style6 p .style4 {
	text-align: right;
...
<div id='cssmenu'>
  ...
  <form id="forma_pagare" name="forma_pagare" method="post" action="<?php echo $editFormAction; ?>">
    <table width="569" border="0">
      <tr>
        <td colspan="4" align="left" bgcolor="#000033" class="style6"><p>
          <input type="submit" name="Editar" id="Editar" value="Guardar" class="style6"/>
          ...
          <input name="total_pagares" type="text" class="style6" id="total_pagares" value="<?php echo number_format($suma_pagares,2);?>" size="20" readonly="readonly"/>
          </p>
          <p><span class="style4">Pagare a la orden de    </span>
            <select name="clave_empresa" class="style6" id="clave_empresa">
            <?php
do {  
?>
            <option value="<?php echo $row_emp2['clave_empresa']?>"<?php if (!(strcmp($row_emp2['clave_empresa'], $clave_temporal))) {echo "selected=\"selected\"";} ?>><?php echo $row_emp2['representante_empresa']?></option>
            <?php
} while ($row_emp2 = mysqli_fetch_assoc($emp2));
  $rows = mysqli_num_rows($emp2);
  if($rows > 0) {
      mysqli_data_seek($emp2, 0);
	  $row_emp2 = mysqli_fetch_assoc($emp2);
  }
?>
            </select>
...
//		}
	...
	if ($totalRows_pagares<>0){
		...
		do {
			...
			//echo $i,"<BR>";
			echo '<td width="90" align="center"> <input type="text" name="v'.$i.'" id="v'.$i.'" value="'.$row_pagares["vence_pagare"].'" class="style6" size="10"/>'.'<button id="trigger'.$i.'" class="style6">...</button></td>';
...
<div id="apDiv1">
  <table width="502" border="1" class="style6">
    <tr>
      ...
      <td width="362" align="left"><?php echo $numero_pagares;  ?>
    ...
    <tr>
      ...
      <td align="left"><?php echo $fecha_del_contrato; ?></td>
    </tr>
    <tr>
      <td bgcolor="#000033" class="style4">Fecha del Primer Pago</td>
      <td align="left"><?php echo $fecha_del_primerpago;?></td>
    </tr>
    <tr>
      <td bgcolor="#000033" class="style4">Cliente</td>
      <td align="left"><?php  echo $row_contra['nombre_cliente']; ?></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td align="left"><?php  echo $row_contra['moneda']; ?></td>
    </tr>
    <tr>
      <td bgcolor="#000033" class="style4">Total Contrato</td>
      <td align="left"><?php  echo "$".number_format($row_contra['ctotal'],2); ?></td>
    </tr>
  </table>
</div>
<p>&nbsp;</p>
